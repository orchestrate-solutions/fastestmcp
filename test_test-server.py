"""
Tests for test-server MCP Server
...
def load_server_module():
    ...
    spec = importlib.util.spec_from_file_location("test_server", "test-server.py")
...
class TestTestserverServer:
    """Test cases for the test-server MCP server"""

    def test_server_creation(self):
        """Test that the MCP server is created successfully"""
        assert app is not None
        assert hasattr(app, 'name')
        assert "test-server" in app.name
